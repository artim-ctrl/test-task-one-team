...
class SiteController extends Controller
{
    ...
    public function actionCalendar(): string
    {
        ...
        $users = User::find()
            ->leftJoin('auth_assignment aa', 'user.id = aa.user_id')
            ->where(['or', ['aa.item_name' => null], ['<>', 'aa.item_name', 'admin']])
            ->all();

        $dataProvider = new ArrayDataProvider([
            'allModels' => array_map(fn (User $user) => [
                '_' => $user->username . ' ' . $user->email,
            ], $users),
        ]);

        return $this->render('calendar', compact('dataProvider', 'model'));
    }
